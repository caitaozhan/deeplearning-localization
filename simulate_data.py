...
# if __name__ == '':
    ...
    template = 'python generate.py -gd -rd data/{} -sl {} -sd {} -rs {} -nt 1 -ntup -mind 1 -vp {} -spt -po {} -ed 10'  # adding -spt and power

    ps = []
    for i, c in enumerate(config):
        if i not in [0, 1]:   # core issue, bus issue ...
            continue
        command = template.format(c[0], c[1], c[2], c[3], c[4], c[4])
        print(command)
